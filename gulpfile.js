// generated on 2015-06-17 using generator-gulp-webapp 1.0.0
'use strict';
...
function lint(files) {
  return function () {
    return _gulp2['default'].src(files).pipe(reload({ stream: true, once: true })).pipe($.eslint()).pipe($.eslint.format()).pipe($['if'](!_browserSync2['default'].active, $.eslint.failAfterError()));
  };
}
